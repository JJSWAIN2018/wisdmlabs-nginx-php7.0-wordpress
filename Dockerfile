...
RUN apt-get install -y python-software-properties software-properties-common && apt-get update && add-apt-repository ppa:ubuntu-wine/ppa -y 
RUN apt-get update
...
RUN apt-get -y install  mysql-client nginx php-fpm
RUN apt-get -y --no-install-recommends install python3 pwgen vim php7.0-cli php7.0-common  php7.0-curl \
        php7.0-json php7.0-opcache  php7.0-readline php7.0-xml  php7.0-zip php7.0-fpm  \
        php7.0-intl php7.0-gd php7.0-mbstring php7.0-soap php7.0-bcmath php7.0-curl php7.0-ldap python-setuptools curl git unzip
RUN apt-get update && apt-get -y install php7.0-mysqli
